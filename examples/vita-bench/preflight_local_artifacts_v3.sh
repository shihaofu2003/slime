...
ARTIFACT_DIR="${PROJECT_ROOT}/output/experiments/vitabench-qwen35-local-role-eval/artifacts-v3"
MANIFEST="${ARTIFACT_DIR}/manifest.json"
SFT_DIR="${ARTIFACT_DIR}/sft"

[[ -x "${VENV_PYTHON}" ]] || { echo "VitaBench Python is unavailable" >&2; exit 1; }
[[ -f "${MANIFEST}" ]] || { echo "schema-6 manifest is unavailable" >&2; exit 1; }
...
if manifest.get("schema_version") != 6:
    raise SystemExit("expected a schema-6 manifest")
execution = manifest.get("execution") or {}
if execution.get("scheduler") != "dynamic_shard_queue":
    raise SystemExit("schema-6 local manifest does not use the dynamic shard queue")
if execution.get("aggregate_max_concurrency") != 16:
    raise SystemExit("schema-6 local manifest does not record concurrency 16")
...
local_files = {
    ...
    "eval_scheduler_sha256": root / "examples/vita-bench/eval_scheduler.py",
}
...
print("SCHEMA6_MANIFEST_PROTOCOL_LOCK_OK", bundle)
PY
...
print("SCHEMA6_GATE_SFT_EXPORT_OK", json.dumps(counts, sort_keys=True))
PY
